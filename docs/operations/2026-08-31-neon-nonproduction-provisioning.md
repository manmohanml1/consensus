...
**Status:** In progress; no migration has run
...
No values are recorded in Git, documentation, issue comments, terminal logs, or
CI. The application does not consume these variables yet.

## Remaining gates

- Create the exact `CONSENSUS_MIGRATION_DATABASE_URL` alias from the managed
  unpooled value in the same Preview and Development scopes. This requires an
  action-time secret-handling confirmation.
- Complete a read-only connectivity query. The Vercel query console requires the
  owner's two-factor verification before it can run.
...
CQ-201 remains open until the exact alias, connectivity, and recovery/teardown
evidence are complete. No Production deployment, tag, release, migration, schema,
or application code change is authorized by this record.
