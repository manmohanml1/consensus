...
3. Store `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID` as environment secrets. `VERCEL_ORG_ID` is Vercel's `team_` identifier; `VERCEL_TEAM_ID` is temporarily accepted only for compatibility.
